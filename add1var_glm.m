function [myGLM, mycoeffs, yhat, resids_glm, rsq] = add1var_glm(O2mat_fglm, O2matraw,distrib, link)
...
if strcmp(link, 'identity')
    yhat = mycoeffs(1) + mycoeffs(2).*O2mat_fglm(:,1) + mycoeffs(3).*O2mat_fglm(:,2); %normal distrib, default link
elseif strcmp(link,'log')
    yhat = exp(mycoeffs(1) + mycoeffs(2).*O2mat_fglm(:,1) + mycoeffs(3).*O2mat_fglm(:,2)); %gamma distrib, log link
elseif strcmp(link, 'reciprocal')
    yhat = 1./(mycoeffs(1) + mycoeffs(2).*O2mat_fglm(:,1) + mycoeffs(3).*O2mat_fglm(:,2)); % reciprocal link
end
% yhat3 = 14.631 - 0.06468.*O2matraw(:,2) + .36851.*new_col; %gamma distrib, identity link (2% smaller r2 than log link)
figure
subplot(2,1,1); plot(O2matraw(:,2),yhat, 'k.')
hold on
plot(O2matraw(:,2), O2matraw(:,1), 'ro')
title('General Linear Model2 PAO2(time, x2)')
xlabel('Time (s)')
ylabel('PAO2 (kPa)')
