function [bhVo2, VO2hypdiff] = extraVO2fromhyps(survey_data, trial_data, quandata2)
...
for ii = 1:length(diffs)
    bhVo2(ii) = (trial_data(regBHnums(ii),17) + new_FRCcol(find(startnewsubj == subjnums(ii))))*(trial_data(regBHnums(ii),2)/(survey_data(regBHnums(ii),3)/10)); %this is max volume of O2 in the lungs at start of normal BH  based on last inhale
    Vo2withH(ii) = (trial_data(regBHnums(ii),17) + new_FRCcol(find(startnewsubj == subjnums(ii))))*((trial_data(regBHnums(ii),2)+currdiff(find(startnewsubj == subjnums(ii))))/(survey_data(regBHnums(ii),3)/10));
end
